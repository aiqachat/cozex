...
<div id="app" v-cloak>
    <app-volcengine-choose @account="changeAccount" :dialog="dialog" url="<?= $url; ?>" @close="closeDialog"
        title="<?= $form->textName($form->ttsLong) ?>"></app-volcengine-choose>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div style="color: #a4a4a4;">
                <?= $form->text($form->ttsLong) ?>
            </div>
        </div>
            ...
            <div class="model-item" flex="dir:left box:first" @click.stop="choose(item)" v-for="item in voices"
                ...
                <div style="flex: 1; display: flex; flex-direction: column;">
                    ...
                    <div style="margin-top: auto; display: flex; justify-content: space-between; align-items: center;">
                        <div></div>
                        <el-button @click.stop="playMusic(item.audition)" v-if="item.audition" class="model-btn"
                            type="info" round size="mini">
                            <i
                                :class="isPlaying && currentAudioUrl === item.audition ? 'el-icon-video-pause' : 'el-icon-video-play'"></i>
                            试听
                        </el-button>
                    </div>
                </div>
            ...
            <div class="voice_text">
                <div class="voice_input" style="margin-top: 10px;">
                    <el-form-item label="" prop="text">
                        <el-input size="small" type="textarea" v-model="data.text" rows="8" maxlength="100000"
                ...
                <div class="voice_input ss">
                    <el-form-item label="" prop="data.speed" class="item" style="margin-left: 10px;">
                        语速（0.2-3 默认为 1）
                        <el-input-number size="small" v-model="data.data.speed" :min="0.2" :max="3"
        ...
        <div class="table-body" style="margin-top: 10px;">
            <div flex="box:last cross:center" style="margin-bottom: 5px;">
                <app-batch-upload :data="uploadData" @success="batchSuccess" :select-data="selectData"
                    :type="<?= $form->ttsLong; ?>"></app-batch-upload>
                <el-button type="primary" size="small"
                    @click="$navigate({r:'netb/volcengine/one', type: <?= $form->ttsLong; ?>})">更多</el-button>
            </div>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            submit() {
                ...
                this.$refs.data.validate((valid) => {
                    if (valid) {
                        ...
                        request({
                            params: { r: 'netb/volcengine/tts-long-text' },
                            method: 'post',
            ...
            getList(type = 1) {
                ...
                request({
                    params: Object.assign({ r: 'netb/volcengine/tts-long-text' }, this.searchData),
                }).then(e => {
                    if (e.data.code === 0) {
