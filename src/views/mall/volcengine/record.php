...
<div id="app" v-cloak>
    <app-volcengine-choose @account="changeAccount" :dialog="newDialog" @close="closeDialog" title="<?=SpeechForm::text_name[$_GET['type']] ?? '语音合成'?>"></app-volcengine-choose>
    <el-alert style="margin-bottom: 10px;" :closable="false"
              type="success">
        <?=SpeechForm::text[$_GET['type']] ?? ''?>
    </el-alert>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            submit() {
                this.$refs.data.validate((valid) => {
                    if (valid) {
                        ...
                        request({
                            params: {r: 'mall/volcengine/record'},
                            method: 'post',
            ...
            getList(type = 1) {
                ...
                let param = Object.assign({r: 'mall/volcengine/record', page: this.page}, this.searchData);
                request({
                    params: param,
                }).then(e => {
                    if (e.data.code === 0) {
