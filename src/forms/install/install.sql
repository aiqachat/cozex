...
CREATE TABLE `wstx_admin_info` (
  ...
  `secondary_permissions` text NOT NULL COMMENT '账户二级权限',
  `remark` varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  `expired_at` timestamp NOT NULL DEFAULT '2025-01-01 00:00:01' COMMENT '账户过期时间',
...
CREATE TABLE `wstx_option` (
  ...
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT '2025-01-01 00:00:01',
  PRIMARY KEY (`id`),
  KEY `name` (`name`),
  KEY `group` (`group`)
...
CREATE TABLE `wstx_user_info` (
  ...
  `set_data` longtext COMMENT '用户设置数据',
  `invite_code` varchar(255) NOT NULL DEFAULT '' COMMENT '邀请码',
  `integral` int(11) NOT NULL DEFAULT '0',
  `total_integral` int(11) NOT NULL DEFAULT '0',
  `award_money` decimal(10,2) NOT NULL DEFAULT '0.00',
  `remark` varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  `is_delete` tinyint(1) NOT NULL DEFAULT '0',
...
CREATE TABLE `wstx_bot_conf` (
    ...
    `show_footer` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否显示底部栏',
    `is_upload` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否允许上传文件',
    `audio_conf` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否允许上传音频',
    `footer_link` varchar(250) DEFAULT '' COMMENT '底部链接',
    `footer_text` varchar(250) DEFAULT '' COMMENT '底部文字',
    `nickname` varchar(50) DEFAULT '' COMMENT '昵称',
    `user_avatar` varchar(250) DEFAULT '' COMMENT '用户头像',
    `mall_id` int(11) NOT NULL DEFAULT '0',
